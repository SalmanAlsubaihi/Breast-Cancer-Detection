...
#         return p_dict
    ...
    def get_class_counts(self):
        counts = self.df['BIRADS'].value_counts()
        class_counts = torch.tensor([counts['BIRADS 0'], counts['BIRADS 1'], counts['BIRADS 2']], dtype=torch.float32)
        return class_counts.max()/class_counts


# TODO: replace 'birad' by 'label' or 'class'
# class BcDatasetLocal(BcDatasetBase):
...
#         # patient_info = self.df[self.df.P_ID == patient_id][['IMAGE_PATH', 'IL', 'VP', 'BIRADS']].values.tolist()
#         patient_info = self.df[self.df.P_ID == patient_id][['IMAGE_PATH', 'IL', 'VP', 'Overall_score']].values.tolist()
#         patient_info = [[path, f'{side}_{view}', self.label_mapping[birads]]
#                         for path, side, view, birads in patient_info]
#         label = torch.tensor(int(patient_info[0][2]), dtype=torch.float32).long()
...
dataset_paths = {
    ...
    'mini_ddsm':{'train_csv_file_path':'MINI-DDSM-Complete-JPEG-8/DataWMask_train_short.csv',
                  'val_csv_file_path':'MINI-DDSM-Complete-JPEG-8/DataWMask_val.csv',
